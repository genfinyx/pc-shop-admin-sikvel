...
export class InvoiceOutTable extends BaseTable {
  constructor() {
    super();
    this.editId = null;
    this.formData = {};
  ...
  async load(page = 1, search = '') {
    ...
    await this.loadData('invoice_out', page, search);
  ...
  async changePage(page) {
    ...
    await renderTable('invoice_out', 'tableContainer', {
  ...
  filterOrders(searchText) {
    ...
    dropdown.innerHTML = filtered.map(order => {
      ...
      return `
        <div class="order-dropdown-item" style="padding: 8px 12px; cursor: pointer; border-bottom: 1px solid #374151;" onclick="tables.invoice_out.selectOrder(${order.order_id}, '${escapedText}')">
          ${displayText}
  ...
  filterProducts(searchText) {
    ...
    dropdown.innerHTML = filtered.map(product => {
      ...
      return `
        <div class="product-dropdown-item" style="padding: 8px 12px; cursor: pointer; border-bottom: 1px solid #374151;" onclick="tables.invoice_out.selectProduct(${product.product_id}, '${escapedText}', ${productPrice})">
          ${displayText}
  ...
  // ========== CRUD ОПЕРАЦИИ ==========
  async createInvoiceOut(data) {
    try {
      const result = await window.go.main.App.CreateInvoiceOut(data);
      if (result.success) {
        this.closeModal();
        await this.load(this.currentPage, this.currentSearch);
        Toast.success('Расходная накладная создана');
        const { renderTable } = await import('../TableContainer.js');
        await renderTable('invoice_out', 'tableContainer', {
          onEdit: (table, id) => console.log('Edit', table, id),
          onDelete: (table, id) => console.log('Delete', table, id)
  ...
  async updateInvoiceOut(id, data) {
    try {
      const result = await window.go.main.App.UpdateInvoiceOut(id, data);
      if (result.success) {
        this.closeModal();
        await this.load(this.currentPage, this.currentSearch);
        Toast.success('Расходная накладная обновлена');
        const { renderTable } = await import('../TableContainer.js');
        await renderTable('invoice_out', 'tableContainer', {
          onEdit: (table, id) => console.log('Edit', table, id),
          onDelete: (table, id) => console.log('Delete', table, id)
  ...
  async deleteInvoiceOut(id) {
    try {
      const result = await window.go.main.App.DeleteInvoiceOut(id);
      if (result.success) {
        await this.load(this.currentPage, this.currentSearch);
        Toast.success('Расходная накладная удалена');
        const { renderTable } = await import('../TableContainer.js');
        await renderTable('invoice_out', 'tableContainer', {
          onEdit: (table, id) => console.log('Edit', table, id),
          onDelete: (table, id) => console.log('Delete', table, id)
  ...
  async openEditForm(id) {
    this.editId = id;
    const data = await this.loadInvoiceOutData(id);
    if (data) {
      this.formData = data;
      this.renderModal();
    }
  ...
  async loadInvoiceOutData(id) {
    try {
      const data = await window.go.main.App.GetInvoiceOut(id);
      return data;
    } catch (error) {
        ...
        <div class="modal-dialog" style="max-width: 400px;">
          <div class="modal-content">
            <div class="modal-header" style="border-bottom-color: #ef4444;">
              ...
              <button type="button" class="btn-close" onclick="tables.invoice_out.closeDeleteModal()">
            ...
            <div class="modal-body">
              <p>Вы уверены, что хотите удалить расходную накладную #${id}?</p>
              <p class="text-secondary">Это действие нельзя отменить.</p>
            </div>
            <div class="modal-footer">
              <button type="button" class="btn btn-cancel" onclick="tables.invoice_out.closeDeleteModal()">Отмена</button>
              <button type="button" class="btn btn-delete" onclick="tables.invoice_out.confirmDelete(${id})">Удалить</button>
            </div>
          </div>
        </div>
      </div>
  ...
  async confirmDelete(id) {
    ...
    await this.deleteInvoiceOut(id);
  }
  ...
  renderModal() {
    const title = this.editId ? 'Редактировать расходную накладную' : 'Новая расходная накладная';
        ...
        <div class="modal-dialog modal-lg">
          <div class="modal-content">
            <div class="modal-header">
              ...
              <button type="button" class="btn-close" onclick="tables.invoice_out.closeModal()">
                <svg width="128" height="128" viewBox="0 0 64 64" stroke="#9ca3af" stroke-width="3px" xmlns="http://www.w3.org/2000/svg"><path d="M22.6066 21.3934C22.2161 21.0029 21.5829 21.0029 21.1924 21.3934C20.8019 21.7839 20.8019 22.4171 21.1924 22.8076L22.6066 21.3934ZM40.9914 42.6066C41.3819 42.9971 42.0151 42.9971 42.4056 42.6066C42.7961 42.2161 42.7961 41.5829 42.4056 41.1924L40.9914 42.6066ZM21.1924 41.1924C20.8019 41.5829 20.8019 42.2161 21.1924 42.6066C21.5829 42.9971 22.2161 42.9971 22.6066 42.6066L21.1924 41.1924ZM42.4056 22.8076C42.7961 22.4171 42.7961 21.7839 42.4056 21.3934C42.0151 21.0029 41.3819 21.0029 40.9914 21.3934L42.4056 22.8076ZM21.1924 22.8076L40.9914 42.6066L42.4056 41.1924L22.6066 21.3934L21.1924 22.8076ZM22.6066 42.6066L42.4056 22.8076L40.9914 21.3934L21.1924 41.1924L22.6066 42.6066Z" fill="black"/></svg>
              </button>
            </div>
            <div class="modal-body">
              <form id="invoiceOutForm" onsubmit="tables.invoice_out.saveForm(event)">
                <div class="row">
                  <div class="col-md-6 mb-3">
                    <label class="form-label">Номер накладной</label>
                    <input type="text" name="invoice_number" class="form-control" value="${this.formData.invoice_number || ''}" placeholder="INV-OUT-...">
                    <span class="text-secondary">Например: INV-OUT-001</span>
                  </div>
                  ...
                  <div class="col-md-6 mb-3">
                    ...
                    <input type="text" id="orderSearchInput" class="form-control" placeholder="Введите номер заказа для поиска..." autocomplete="off" oninput="tables.invoice_out.filterOrders(this.value)" value="${this.getOrderNumberForInput(this.formData.order_id)}">
                  ...
                  <div class="col-md-6 mb-3">
                    ...
                    <input type="text" id="productSearchInput" class="form-control" placeholder="Введите название товара для поиска..." autocomplete="off" oninput="tables.invoice_out.filterProducts(this.value)" value="${this.getProductNameForInput(this.formData.product_id)}">
            ...
            <div class="modal-footer">
              <button type="button" class="btn btn-cancel" onclick="tables.invoice_out.closeModal()">Отмена</button>
              <button type="submit" class="btn btn-save" form="invoiceOutForm">Сохранить</button>
            </div>
          </div>
        </div>
      </div>
  ...
  async saveForm(e) {
    ...
    if (this.editId) {
      await this.updateInvoiceOut(this.editId, data);
    } else {
      await this.createInvoiceOut(data);
    }
  }
  ...
  formatCellValue(column, value) {
    ...
    if (column === 'invoice_date' || column === 'created_at') return this.formatDateTime(value);
  ...
  renderRow(row, columns) {
    ...
    <tr>
      ${columns.map(col => {
      if (col === 'price') {
        return `<td>${Number(row.price || 0).toFixed(2)} ₽</td>`;
      }
      return `<td>${this.formatCellValue(col, row[col])}</td>`;
    }).join('')}
      <td class="text-end">
        <button class="btn btn-edit" onclick="tables.invoice_out.openEditForm(${row.invoice_out_id})">Изменить</button>
        <button class="btn btn-delete" onclick="tables.invoice_out.showDeleteModal(${row.invoice_out_id})">Удалить</button>
      </td>
    </tr>
  `;
  }
  ...
  render(options = {}) {
    ...
    if (!this.data.rows || this.data.rows.length === 0) {
      return this.renderEmptyState('Расходные накладные');
    }
      ...
      <div class="table-card">
        <div class="table-header">
          ...
          <div class="table-header-right">
            ...
            <button class="btn btn-primary" onclick="tables.invoice_out.openCreateForm()">+ Добавить</button>
          </div>
        </div>
...
window.tables.invoice_out = new InvoiceOutTable();