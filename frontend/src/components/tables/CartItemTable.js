...
export class CartItemTable extends BaseTable {
  constructor() {
    super();
    this.editId = null;
    this.formData = {};
  ...
  async load(page = 1, search = '') {
    ...
    await this.loadData('cart_item', page, search);
  ...
  async changePage(page) {
    ...
    await renderTable('cart_item', 'tableContainer', {
  ...
  filterUsers(searchText) {
    ...
    dropdown.innerHTML = filtered.map(user => {
      ...
      return `
        <div class="user-dropdown-item" style="padding: 8px 12px; cursor: pointer; border-bottom: 1px solid #374151;" onclick="tables.cart_item.selectUser(${user.idUser}, '${escapedText}')">
          ${displayText}
  ...
  filterProducts(searchText) {
    ...
    dropdown.innerHTML = filtered.map(product => {
      ...
      return `
        <div class="product-dropdown-item" style="padding: 8px 12px; cursor: pointer; border-bottom: 1px solid #374151;" onclick="tables.cart_item.selectProduct(${product.product_id}, '${escapedText}', ${productPrice})">
          ${displayText}
  ...
  async createCartItem(data) {
    try {
      const result = await window.go.main.App.CreateCartItem(data);
      if (result.success) {
        this.closeModal();
        await this.load(this.currentPage, this.currentSearch);
  ...
  async updateCartItem(id, data) {
    try {
      const result = await window.go.main.App.UpdateCartItem(id, data);
      if (result.success) {
        this.closeModal();
        await this.load(this.currentPage, this.currentSearch);
        Toast.success('Позиция корзины обновлена');
        const { renderTable } = await import('../TableContainer.js');
        await renderTable('cart_item', 'tableContainer', {
          onEdit: (table, id) => console.log('Edit', table, id),
          onDelete: (table, id) => console.log('Delete', table, id)
  ...
  async deleteCartItem(id) {
    try {
      const result = await window.go.main.App.DeleteCartItem(id);
      if (result.success) {
        await this.load(this.currentPage, this.currentSearch);
        Toast.success('Позиция корзины удалена');
        const { renderTable } = await import('../TableContainer.js');
        await renderTable('cart_item', 'tableContainer', {
          onEdit: (table, id) => console.log('Edit', table, id),
          onDelete: (table, id) => console.log('Delete', table, id)
  ...
  async openEditForm(id) {
    this.editId = id;
    const data = await this.loadCartItemData(id);
    if (data) {
      this.formData = data;
      this.renderModal();
    }
  ...
  async loadCartItemData(id) {
    try {
      const data = await window.go.main.App.GetCartItem(id);
      return data;
    } catch (error) {
      Toast.error('Ошибка загрузки данных: ' + error.message);
      return null;
        ...
        <div class="modal-dialog" style="max-width: 400px;">
          <div class="modal-content">
            <div class="modal-header" style="border-bottom-color: #ef4444;">
              ...
              <button type="button" class="btn-close" onclick="tables.cart_item.closeDeleteModal()">
            ...
            <div class="modal-body">
              <p>Вы уверены, что хотите удалить позицию корзины #${id}?</p>
              <p class="text-secondary">Это действие нельзя отменить.</p>
            </div>
            <div class="modal-footer">
              <button type="button" class="btn btn-cancel" onclick="tables.cart_item.closeDeleteModal()">Отмена</button>
              <button type="button" class="btn btn-delete" onclick="tables.cart_item.confirmDelete(${id})">Удалить</button>
            </div>
          </div>
        </div>
      </div>
  ...
  renderModal() {
    const title = this.editId ? 'Редактировать позицию корзины' : 'Добавить товар в корзину';
        ...
        <div class="modal-dialog modal-lg">
          <div class="modal-content">
            <div class="modal-header">
              ...
              <button type="button" class="btn-close" onclick="tables.cart_item.closeModal()">
                <svg width="128" height="128" viewBox="0 0 64 64" stroke="#9ca3af" stroke-width="3px" xmlns="http://www.w3.org/2000/svg">
                  <path d="M22.6066 21.3934C22.2161 21.0029 21.5829 21.0029 21.1924 21.3934C20.8019 21.7839 20.8019 22.4171 21.1924 22.8076L22.6066 21.3934ZM40.9914 42.6066C41.3819 42.9971 42.0151 42.9971 42.4056 42.6066C42.7961 42.2161 42.7961 41.5829 42.4056 41.1924L40.9914 42.6066ZM21.1924 41.1924C20.8019 41.5829 20.8019 42.2161 21.1924 42.6066C21.5829 42.9971 22.2161 42.9971 22.6066 42.6066L21.1924 41.1924ZM42.4056 22.8076C42.7961 22.4171 42.7961 21.7839 42.4056 21.3934C42.0151 21.0029 41.3819 21.0029 40.9914 21.3934L42.4056 22.8076ZM21.1924 22.8076L40.9914 42.6066L42.4056 41.1924L22.6066 21.3934L21.1924 22.8076ZM22.6066 42.6066L42.4056 22.8076L40.9914 21.3934L21.1924 41.1924L22.6066 42.6066Z" fill="none" stroke="#9ca3af"/>
                </svg>
              </button>
            ...
            <div class="modal-body">
              <form id="cartItemForm" onsubmit="tables.cart_item.saveForm(event)">
                <div class="row">
                  <div class="col-md-6 mb-3">
                    <label class="form-label">Пользователь <span class="text-danger">*</span></label>
                    <input type="text" id="userSearchInput" class="form-control" placeholder="Введите имя, фамилию или логин для поиска..." autocomplete="off" oninput="tables.cart_item.filterUsers(this.value)" value="${this.getUserNameForInput(this.formData.user_id)}">
                  ...
                  <div class="col-md-6 mb-3">
                    ...
                    <input type="text" id="productSearchInput" class="form-control" placeholder="Введите название товара для поиска..." autocomplete="off" oninput="tables.cart_item.filterProducts(this.value)" value="${this.getProductNameForInput(this.formData.product_id)}">
            ...
            <div class="modal-footer">
              <button type="button" class="btn btn-cancel" onclick="tables.cart_item.closeModal()">Отмена</button>
              <button type="submit" class="btn btn-save" form="cartItemForm">Сохранить</button>
            </div>
          </div>
        </div>
      </div>
  ...
  async saveForm(e) {
    ...
    if (this.editId) {
      await this.updateCartItem(this.editId, data);
    } else {
      await this.createCartItem(data);
    }
  }
  ...
  renderRow(row, columns) {
    return `
      <tr>
        ...
        <td class="text-end">
          <button class="btn btn-edit" onclick="tables.cart_item.openEditForm(${row.cart_item_id})">Изменить</button>
          <button class="btn btn-delete" onclick="tables.cart_item.showDeleteModal(${row.cart_item_id})">Удалить</button>
        </td>
      </tr>
    `;
  }
  ...
  render(options = {}) {
    ...
    if (!this.data.rows || this.data.rows.length === 0) {
      return this.renderEmptyState('Корзина');
    }
      ...
      <div class="table-card">
        <div class="table-header">
          ...
          <div class="table-header-right">
            ...
            <button class="btn btn-primary" onclick="tables.cart_item.openCreateForm()">+ Добавить</button>
          </div>
        </div>
...
window.tables.cart_item = new CartItemTable();